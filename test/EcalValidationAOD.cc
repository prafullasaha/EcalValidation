...
{
//cout << "begin analyze function" << endl;
nPU_   .clear();    
puTrue_.clear();
puBX_  .clear();
...
//PileUp interactions
//if(vertexes->size() ==1) cout << "cleaned verticies " << nvertices << endl;  
if(!nvertices) return;
double MyWeight(0);
if(isMC){
...
//cout <<MyWeight << "\t" <<  LumiWeights[pu->getPU_NumInteractions()+20] << "\t" <<  LumiWeights[pu->getPU_NumInteractions()+40] << endl;
 ...
 if(nvertices) MyWeight = LumiWeights[nvertices];
...
if(itr->checkFlag(itr->kWeird)) continue;
	...
	h_recHits_EB_energy_spike       -> Fill( itr -> energy(), MyWeight);
      ...
      // spike cleaning
      if ( R4 > 0.95 && et > 4.) hasSpike= true;
      if ( R4 > 0.95 && et > 4.) continue;
...
if( !(itr->checkFlag(itr->kOutOfTime))   &&  !(itr->checkFlag(itr->kWeird)))
	...
	h_recHits_EB_iEtaOccupancy -> Fill( ebid.ieta(), MyWeight);
      ...
      if ( (itr -> energy() > ethrEB_)  && !hasSpike){
...
if(maxRecHitEnergyEB >=2.0){ 
  h_recHits_EB_size           -> Fill( recHitsEB->size(), MyWeight);
  if (!hasSpike) h_recHits_EB_size_cleaned -> Fill( recHitsEB->size(), MyWeight);
  ...
  // size
  h_recHits_EE_size    -> Fill( recHitsEE->size(), MyWeight);
if(recHitsEE->size() ==0) 
