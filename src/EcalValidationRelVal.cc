...
#include "Validation/EcalValidation/interface/EcalValidationRelVal.h"
...
//
EcalValidationRelVal::EcalValidationRelVal(const edm::ParameterSet& ps)
{
...
EcalValidationRelVal::~EcalValidationRelVal()
{
...
void EcalValidationRelVal::analyze(const edm::Event& ev, const edm::EventSetup& iSetup)
{
...
// ------------ method called once each job just before starting event loop  ------------
        void 
EcalValidationRelVal::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
EcalValidationRelVal::endJob() {
...
void EcalValidationRelVal::doPi0Barrel ( const CaloGeometry *geometry,
...
void EcalValidationRelVal::doPi0Endcap ( const CaloGeometry *geometry,
				   const CaloTopology *topology,
				   edm::Handle<EcalRecHitCollection> recHitsEE ) 
{
  const CaloSubdetectorGeometry *geometry_EE = geometry->getSubdetectorGeometry(DetId::Ecal,EcalEndcap);
...
void EcalValidationRelVal::convxtalid(Int_t &nphi,Int_t &neta)
{
  // Barrel only
...
int EcalValidationRelVal::diff_neta_s(Int_t neta1, Int_t neta2){
  Int_t mdiff;
  mdiff=(neta1-neta2);
  return mdiff;
}
...
int EcalValidationRelVal::diff_nphi_s(Int_t nphi1,Int_t nphi2) {
  Int_t mdiff;
...
DEFINE_FWK_MODULE(EcalValidationRelVal);
