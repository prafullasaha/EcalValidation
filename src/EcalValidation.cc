...
EcalValidation::EcalValidation(const edm::ParameterSet& ps)
{
 ...
 //ebEcalDigiCollection_     = consumes<EBSrFlagCollection> (ps.getParameter<edm::InputTag>("ebEcalDigiCollection"));
 //eeEcalDigiCollection_     = consumes<EESrFlagCollection> (ps.getParameter<edm::InputTag>("eeEcalDigiCollection"));
  ...
  h_recHits_EB_eneVSieta   = fs->make<TH2D>("h_recHits_EB_eneVSieta", "h_recHits_EB_eneVSieta", 11000,-50,500, 172,-86.,86.);   
  h_recHits_EB_eneVSchi2   = fs->make<TH2D>("h_recHits_EB_eneVSchi2", "h_recHits_EB_eneVSchi2", 11000,-50,500,1000,0,100); 
  h_recHits_EB_chi2VSieta  = fs->make<TH2D>("h_recHits_EB_chi2VSieta","h_recHits_EB_chi2VSieta",1000,0,100, 172,-86.,86.);
  ...
  h_recHits_EEP_eneVSix   = fs->make<TH2D>("h_recHits_EEP_eneVSix", "h_recHits_EEP_eneVSix", 11000,-50,500, 100,0,100.);   
  h_recHits_EEP_eneVSchi2ix   = fs->make<TH2D>("h_recHits_EEP_eneVSchi2ix", "h_recHits_EEP_eneVSchi2ix", 11000,-50,500,1000,0,100); 
  h_recHits_EEP_chi2VSix  = fs->make<TH2D>("h_recHits_EEP_chi2VSix","h_recHits_EEP_chi2VSix",1000,0,100, 100, 0, 100);
  h_recHits_EEP_eneVSiy   = fs->make<TH2D>("h_recHits_EEP_eneVSiy", "h_recHits_EEP_eneVSiy", 11000,-50,500, 100,0,100.);   
  h_recHits_EEP_eneVSchi2iy   = fs->make<TH2D>("h_recHits_EEP_eneVSchi2iy", "h_recHits_EEP_eneVSchi2iy", 11000,-50,500,1000,0,100); 
  h_recHits_EEP_chi2VSiy  = fs->make<TH2D>("h_recHits_EEP_chi2VSiy","h_recHits_EEP_chi2VSiy",1000,0,100, 100, 0, 100);

//nRecHits_EB;
//nRecHits_EEP;
//nRecHits_EEM;
  RecHitVSEta_EEP            = fs->make<TH2D>("RecHitVSEta_EEP ","RecHitVSEta_EEP",1000,0,10000, 300,-3.,3. );
  RecHitVSEta_EEM            = fs->make<TH2D>("RecHitVSEta_EEM ","RecHitVSEta_EEM",1000,0,10000, 300,-3.,3. );
  RecHitVSPhi_EEP            = fs->make<TH2D>("RecHitVSPhi_EEP ","RecHitVSPhi_EEP",1000,0,10000,360,-3.1415927, 3.1415927 );
  RecHitVSPhi_EEM            = fs->make<TH2D>("RecHitVSPhi_EEM ","RecHitVSPhi_EEM",1000,0,10000,360,-3.1415927, 3.1415927 );
  ...
  h_recHits_EEM_eneVSix       = fs->make<TH2D>("h_recHits_EEM_eneVSix", "h_recHits_EEM_eneVSix", 11000,-50,500, 100,0,100.);   
  h_recHits_EEM_eneVSchi2ix   = fs->make<TH2D>("h_recHits_EEM_eneVSchi2ix", "h_recHits_EEM_eneVSchi2ix", 11000,-50,500,1000,0,100); 
  h_recHits_EEM_chi2VSix      = fs->make<TH2D>("h_recHits_EEM_chi2VSix","h_recHits_EEM_chi2VSix",1000,0,100, 100, 0, 100);
  h_recHits_EEM_eneVSiy       = fs->make<TH2D>("h_recHits_EEM_eneVSiy", "h_recHits_EEM_eneVSiy", 11000,-50,500, 100,0,100.);   
  h_recHits_EEM_eneVSchi2iy   = fs->make<TH2D>("h_recHits_EEM_eneVSchi2iy", "h_recHits_EEM_eneVSchi2iy", 11000,-50,500,1000,0,100); 
  h_recHits_EEM_chi2VSiy      = fs->make<TH2D>("h_recHits_EEM_chi2VSiy","h_recHits_EEM_chi2VSiy",1000,0,100, 100, 0, 100);
...
{
//if(ev.id().luminosityBlock() !=124) return;
...
//   float orbitNb = ev.orbitNumber();
  ...
  //edm::Handle<EBSrFlagCellection> SRFlagsEB;
  //SaveSrFlag_ = false;
 int nvertices(0); 
////  //EB SR FLAGS    
...
////if(nvertices !=20) continue; 
  ...
  //if(SaveSrFlag_)
  //{
  //   ev.getByToken(eeEcalDigiCollection_, SRFlagsEE );
  //
...
//   const CaloSubdetectorGeometry *geometry_EE = geometry->getSubdetectorGeometry(DetId::Ecal,EcalEndcap);
  ...
  for ( EcalRecHitCollection::const_iterator itr = theBarrelEcalRecHits->begin () ;
	...
	h_recHits_EB_eneVSchi2      -> Fill( itr->energy() , itr->chi2() );
	h_recHits_EB_chi2VSieta      -> Fill( itr->chi2() , ebid.ieta() );
  ...
  for ( EcalRecHitCollection::const_iterator itr = theEndcapEcalRecHits->begin () ;
	...
	nHitsEEP++;
RecHitVSEta_EEP->Fill(itr->chi2(), mycell.eta());
RecHitVSPhi_EEP->Fill(itr->chi2(), mycell.phi());
	h_recHits_EEP_energy        -> Fill( itr -> energy() );
        h_recHits_EE_recoFlag       -> Fill( itr -> recoFlag() );
        h_recHits_recoFlag          -> Fill( itr -> recoFlag() );
         h_recHits_EEP_eneVSix    ->Fill(itr->energy(), eeid.ix());
         h_recHits_EEP_eneVSchi2ix->Fill(itr->energy(), itr->chi2());
         h_recHits_EEP_chi2VSix   ->Fill(itr->chi2(), eeid.ix());
         h_recHits_EEP_eneVSiy    ->Fill(itr->energy(), eeid.iy());
         h_recHits_EEP_eneVSchi2iy->Fill(itr->energy(), itr->chi2());
         h_recHits_EEP_chi2VSiy   ->Fill(itr->chi2(), eeid.iy());
	...
	nHitsEEM++;
RecHitVSEta_EEM->Fill(itr->chi2(), mycell.eta());
RecHitVSPhi_EEM->Fill(itr->chi2(), mycell.phi());
	...
	if (et > maxEtRecHitEnergyEEM && 
	  ...
	  eeid_MrecHitEEM      = eeid ;
         ...
         h_recHits_EEM_eneVSix    ->Fill(itr->energy(), eeid.ix());
         h_recHits_EEM_eneVSchi2ix->Fill(itr->energy(), itr->chi2());
         h_recHits_EEM_chi2VSix   ->Fill(itr->chi2(), eeid.ix());
         h_recHits_EEM_eneVSiy    ->Fill(itr->energy(), eeid.iy());
         h_recHits_EEM_eneVSchi2iy->Fill(itr->energy(), itr->chi2());
         h_recHits_EEM_chi2VSiy   ->Fill(itr->chi2(), eeid.iy());
	// only channels above noise
	if (  itr -> energy() > ethrEE_ ) {
...
//cout << "number of rechits EE- and EE+ " << nHitsEEP << "\t" << nHitsEEM << endl;
